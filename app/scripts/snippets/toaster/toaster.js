(function () {
  ...
  angular.module('toaster', ['ngAnimate']).constant(
    'toasterConfig', {
      ...
      // Options: '', 'trustedHtml', 'template', 'templateWithData'
      'body-output-type': '',
    ...
    'toasterEventRegistry', ToasterEventRegistry
  ).directive(
    'toasterContainer', ToasterContainer
  ).controller(
    'ToasterContainerCtrl', ToasterContainerController
  );

  ToasterContainer.$inject = ['$parse', '$interval', '$sce', 'toasterConfig', 'toaster', 'toasterEventRegistry'];
  function ToasterContainer($parse, $interval, $sce, toasterConfig, toaster, toasterEventRegistry) {
    return {
      replace: true,
      restrict: 'EA',
      ...
      link: function (scope, elm, attrs) {
        ...
        function setTimeout1(toast, time) {
        ...
        scope.configureTimer = function (toast) {
          ...
          if (timeout > 0) {
            setTimeout1(toast, timeout);
          }
        };

        function addToast(toast, toastId) {
          var i, len, closeButton, closeButtonForType, fcGet, templateWithData;
          ...
          if (typeof toast.showCloseButton === 'boolean') {
            ...
            closeButtonForType = closeButton[toast.type];
          ...
          switch (toast.bodyOutputType) {
            ...
            case 'templateWithData':
              fcGet = $parse(toast.body || mergedConfig['body-template']);
              templateWithData = fcGet(scope);
        ...
        function removeAllToasts(toastId) {
          var i;
          for (i = scope.toasters.length - 1; i >= 0; i--) {
            if (isUndefinedOrNull(toastId)) {
              removeToast(i);
            } else if (scope.toasters[i].uid === toastId) {
              removeToast(i);
            }
          }
        }
      ...
      controller: 'ToasterContainerCtrl',
  ...
  ToasterContainerController.$inject = ['$scope', '$interval'];
  function ToasterContainerController($scope, $interval) {
    // Called on mouseover
  ...
  ToasterEventRegistry.$inject = ['$rootScope'];
  function ToasterEventRegistry($rootScope) {
        ...
        if (!deregisterNewToast) {
          deregisterNewToast = $rootScope.$on(
            'toaster-newToast', function (event, toasterId, toastId) {
              var i, len = newToastEventSubscribers.length;
              for (i = 0; i < len; i++) {
        ...
        if (!deregisterClearToasts) {
          deregisterClearToasts = $rootScope.$on(
            'toaster-clearToasts', function (event, toasterId, toastId) {
              var i, len = clearToastsEventSubscribers.length;
              for (i = 0; i < len; i++) {
  ...
  ToasterService.$inject = ['$rootScope', 'toasterConfig'];
  function ToasterService($rootScope, toasterConfig) {
    var typeIconClass;
    this.pop = function (type, title, body, timeout, bodyOutputType, clickHandler, toasterId, showCloseButton, toastId) {
      var params;
      if (angular.isObject(type)) {
        // Enable named parameters as pop argument
        params = type;
        this.toast = {
          type: params.type,
          title: params.title,
    ...
    for (typeIconClass in toasterConfig['icon-classes']) {
      this[typeIconClass] = (function (toasterType) {
